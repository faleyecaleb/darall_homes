        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-end">
            ...
            <div class="lg:col-span-8 flex flex-col items-start gap-4 sm:gap-6">
                ...
                <div class="flex flex-col gap-2">
                    <span class="text-sm text-slate-400 uppercase tracking-widest font-semibold">{{ $property->location->name }}</span>
            ...
            <div class="lg:col-span-4 flex flex-col lg:items-end gap-2 text-left lg:text-right">
                ...
                @if($property->property_type === 'Shortlet')
                    <span class="text-3xl sm:text-4xl font-bold text-amber-500">₦{{ number_format($property->price) }}<span class="text-sm font-light text-slate-400"> / night</span></span>
                @else
                    <span class="text-3xl sm:text-4xl font-bold text-amber-500">₦{{ number_format($property->price) }}</span>
                @endif
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="grid grid-cols-2 md:grid-cols-5 gap-8 text-center divide-x divide-slate-100">
            ...
            <div class="flex flex-col gap-1 items-center col-span-2 md:col-span-1 border-t md:border-t-0 pt-4 md:pt-0">
                <span class="text-slate-400 text-xs uppercase tracking-wider font-semibold">Purpose</span>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-16">
            ...
            <div class="lg:col-span-8 flex flex-col gap-6">
                <span class="text-amber-600 font-semibold tracking-wider text-xs uppercase">Property Overview</span>
                <h2 class="text-3xl font-serif text-slate-900">Architectural Narrative</h2>
            ...
            <!-- Side Specs panel -->
            <div class="lg:col-span-4 bg-slate-50 p-8 rounded-3xl border border-slate-100/50 flex flex-col gap-6 self-start">
                <h3 class="text-lg font-bold text-slate-900">Showroom Identity</h3>
                <div class="space-y-4 text-sm">
                    <div class="flex justify-between border-b border-slate-200/40 pb-3">
                        <span class="text-slate-500">Listing Status</span>
                        <span class="font-bold text-emerald-600 uppercase text-xs">{{ $property->status }}</span>
                    </div>
                    <div class="flex justify-between border-b border-slate-200/40 pb-3">
                        <span class="text-slate-500">Reference ID</span>
                        <span class="font-bold text-slate-900 font-sans">#DR-{{ str_pad($property->id, 4, '0', STR_PAD_LEFT) }}</span>
                    </div>
                    <div class="flex justify-between border-b border-slate-200/40 pb-3">
                        <span class="text-slate-500">Listed Date</span>
                        <span class="font-bold text-slate-900">{{ $property->created_at->format('M d, Y') }}</span>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-8">
                @foreach($property->amenities as $amenity)
                    <div class="flex items-start gap-4 bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
                        ...
                        <div>
                            <h4 class="text-slate-900 font-semibold text-sm">{{ $amenity->name }}</h4>
                            <p class="text-[10px] text-slate-400 mt-0.5">Fully Serviced</p>
                        </div>
                    </div>
                @endforeach
            </div>
            ...
            <form action="{{ route('properties.book', $property->slug) }}" method="POST" class="space-y-6">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    <div class="flex flex-col gap-1.5">
                        ...
                        <input type="text" name="name" required value="{{ old('name') }}" placeholder="John Doe" class="w-full bg-white border border-slate-200 rounded-xl px-4 py-3 text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-transparent transition-all">
                    ...
                    <div class="flex flex-col gap-1.5">
                        ...
                        <input type="email" name="email" required value="{{ old('email') }}" placeholder="john@example.com" class="w-full bg-white border border-slate-200 rounded-xl px-4 py-3 text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-transparent transition-all">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    <div class="flex flex-col gap-1.5">
                        ...
                        <input type="tel" name="phone" required value="{{ old('phone') }}" placeholder="+234 800 0000" class="w-full bg-white border border-slate-200 rounded-xl px-4 py-3 text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-transparent transition-all">
                        @error('phone')<span class="text-xs text-rose-500 font-semibold">{{ $message }}</span>@enderror
                    </div>
                    <div class="flex grid grid-cols-2 gap-4">
                        <div class="flex flex-col gap-1.5">
                            <label class="text-xs font-semibold text-slate-500 uppercase">Preferred Date</label>
                            <input type="date" name="requested_date" required value="{{ old('requested_date') }}" class="w-full bg-white border border-slate-200 rounded-xl px-3 py-3 text-xs text-slate-700 focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-transparent transition-all font-sans">
                            @error('requested_date')<span class="text-xs text-rose-500 font-semibold">{{ $message }}</span>@enderror
                        </div>
                        <div class="flex flex-col gap-1.5">
                            <label class="text-xs font-semibold text-slate-500 uppercase">Preferred Time</label>
                            <select name="requested_time" required class="w-full bg-white border border-slate-200 rounded-xl px-3 py-3.5 text-xs text-slate-700 focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-transparent transition-all">
                            ...
                            @error('requested_time')<span class="text-xs text-rose-500 font-semibold">{{ $message }}</span>@enderror
                        </div>
                    </div>
                </div>
                ...
                <div class="flex flex-col gap-1.5">
                    ...
                    <textarea name="notes" rows="4" placeholder="Would love to request a dusk viewing for sunset analysis..." class="w-full bg-white border border-slate-200 rounded-xl px-4 py-3 text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-transparent transition-all">{{ old('notes') }}</textarea>
