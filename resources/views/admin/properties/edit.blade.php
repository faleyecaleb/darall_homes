...
@section('title', 'Modify Listing: ' . $property->title)
...
<div class="flex flex-col gap-1 mb-10 animate-fade-in duration-500">
    <h1 class="text-3xl font-extrabold tracking-tight text-[#1c1c1e] font-sans">
        Modify Showroom Listing
    </h1>
    <p class="text-sm text-slate-500 font-light">Update spatial metadata, adjust pricing tiers, or update the embedded Matterport tour.</p>
</div>
    ...
    <form action="{{ route('admin.properties.update', $property->id) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-8">
            <div class="flex flex-col gap-2">
                ...
                <input type="text" name="title" required value="{{ old('title', $property->title) }}" placeholder="e.g. The Sapphire Penthouse" class="w-full bg-slate-50 border border-slate-100 rounded-2xl px-5 py-4 text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-[#0d6e60] focus:bg-white transition-all">
            ...
            <div class="flex flex-col gap-2">
                ...
                <input type="number" name="price" required value="{{ old('price', (int)$property->price) }}" placeholder="e.g. 350000000" class="w-full bg-slate-50 border border-slate-100 rounded-2xl px-5 py-4 text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-[#0d6e60] focus:bg-white transition-all font-sans">
        ...
        <div class="flex flex-col gap-2">
            ...
            <textarea name="description" rows="5" placeholder="Bespoke engineering description detailing finishings, ceiling heights, automated systems..." class="w-full bg-slate-50 border border-slate-100 rounded-2xl px-5 py-4 text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-[#0d6e60] focus:bg-white transition-all font-sans">{{ old('description', $property->description) }}</textarea>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-8 font-sans">
            ...
            <div class="flex flex-col gap-2">
                ...
                <select name="property_type" id="property_type_select" required class="w-full bg-slate-50 border border-slate-100 rounded-2xl px-5 py-4 text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-[#0d6e60] focus:bg-white transition-all">
                    <option value="Sale" {{ old('property_type', $property->property_type) === 'Sale' && !in_array($property->status, ['Sold', 'Sold Out', 'Rented', 'Rented Out', 'In Development', 'Under Construction']) ? 'selected' : '' }}>For Sale</option>
                    <option value="Rent" {{ old('property_type', $property->property_type) === 'Rent' && !in_array($property->status, ['Sold', 'Sold Out', 'Rented', 'Rented Out', 'In Development', 'Under Construction']) ? 'selected' : '' }}>For Rent</option>
                    <option value="Shortlet" {{ old('property_type', $property->property_type) === 'Shortlet' && !in_array($property->status, ['Sold', 'Sold Out', 'Rented', 'Rented Out', 'In Development', 'Under Construction']) ? 'selected' : '' }}>Shortlet Stay</option>
                    <option value="In Development" {{ in_array(old('property_type', $property->property_type), ['In Development', 'Under Construction']) || in_array(old('status', $property->status), ['In Development', 'Under Construction']) ? 'selected' : '' }}>In Development</option>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-4 gap-8 font-sans">
            <div class="flex flex-col gap-2">
                ...
                <input type="number" name="bedrooms" required value="{{ old('bedrooms', $property->bedrooms) }}" min="0" class="w-full bg-slate-50 border border-slate-100 rounded-2xl px-5 py-4 text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-[#0d6e60] focus:bg-white transition-all font-sans">
            ...
            <div class="flex flex-col gap-2">
                ...
                <input type="number" name="bathrooms" required value="{{ old('bathrooms', $property->bathrooms) }}" min="0" class="w-full bg-slate-50 border border-slate-100 rounded-2xl px-5 py-4 text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-[#0d6e60] focus:bg-white transition-all font-sans">
            ...
            <div class="flex flex-col gap-2">
                ...
                <input type="number" name="floor_area" value="{{ old('floor_area', $property->floor_area) }}" placeholder="e.g. 450" class="w-full bg-slate-50 border border-slate-100 rounded-2xl px-5 py-4 text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-[#0d6e60] focus:bg-white transition-all font-sans">
            ...
            <div class="flex flex-col gap-2">
                ...
                <select name="status" id="property_status_select" required class="w-full bg-slate-50 border border-slate-100 rounded-2xl px-5 py-4 text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-[#0d6e60] focus:bg-white transition-all">
                    ...
                    <option value="In Development" {{ in_array(old('status', $property->status), ['In Development', 'Under Construction']) || in_array(old('property_type', $property->property_type), ['In Development', 'Under Construction']) ? 'selected' : '' }}>In Development</option>
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
                @foreach($amenities as $amenity)
                    <label class="flex items-center gap-3 bg-white p-4 rounded-2xl border border-slate-200/50 hover:border-[#0d6e60]/30 hover:bg-slate-50/20 cursor-pointer select-none transition-all duration-150 transform hover:scale-[1.01]">
                        <!-- Syncing with is_cover and contains -->
                        <input type="checkbox" name="amenities[]" value="{{ $amenity->id }}" {{ $property->amenities->contains($amenity->id) || (is_array(old('amenities')) && in_array($amenity->id, old('amenities'))) ? 'checked' : '' }} class="h-5 w-5 rounded text-[#0d6e60] focus:ring-[#0d6e60] border-slate-200 cursor-pointer">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-8 font-sans">
            ...
            <div class="flex flex-col gap-2 bg-slate-50 p-6 rounded-3xl border border-slate-100/70" x-data="{ imageSource: '{{ old('cover_image_file') ? 'file' : ($property->coverImage && !str_starts_with($property->coverImage->file_path, '/storage/') ? 'url' : 'file') }}' }">
                ...
                <div x-show="imageSource === 'url'">
                    <input type="url" name="cover_image_url" value="{{ old('cover_image_url', $property->coverImage && !str_starts_with($property->coverImage->file_path, '/storage/') ? $property->coverImage->file_path : '') }}" placeholder="https://images.unsplash.com/photo..." :disabled="imageSource !== 'url'" class="w-full bg-white border border-slate-200 rounded-xl px-5 py-4 text-xs text-slate-700 focus:outline-none focus:ring-2 focus:ring-[#0d6e60] transition-all font-sans">
                </div>
                ...
                <div x-show="imageSource === 'file'" style="display: none;">
                    ...
                    @if($property->coverImage && str_starts_with($property->coverImage->file_path, '/storage/'))
                        <div class="mt-2 text-xs text-slate-400 flex items-center gap-1.5 font-sans">
                            <span class="inline-block w-1.5 h-1.5 rounded-full bg-emerald-500"></span>
                            Current file: <span class="font-mono text-[10px]">{{ basename($property->coverImage->file_path) }}</span>
                        </div>
                    @endif
                </div>
            ...
            <div class="flex flex-col gap-2">
                ...
                <input type="url" name="virtual_tour_url" value="{{ old('virtual_tour_url', $property->virtualTour ? $property->virtualTour->tour_url : '') }}" placeholder="https://my.matterport.com/show/?m=..." class="w-full bg-slate-50 border border-slate-100 rounded-2xl px-5 py-4 text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-[#0d6e60] focus:bg-white transition-all font-sans font-sans">
        ...
        <div class="flex items-center gap-3 bg-slate-50 p-4 rounded-2xl border border-slate-100 max-w-sm font-sans mb-8">
            <input type="checkbox" name="is_featured" id="is_featured" value="1" {{ old('is_featured', $property->is_featured) ? 'checked' : '' }} class="h-5 w-5 rounded text-[#0d6e60] focus:ring-[#0d6e60] border-slate-200 cursor-pointer">
        ...
        @php
            $existingUnits = $property->units->map(function ($u) {
                return [
                    'id' => $u->id,
                    'name' => $u->name,
                    'badge' => $u->badge,
                    ...
                    'floor_area' => $u->floor_area,
                    'outright_price' => $u->outright_price,
                    'has_installment' => $u->has_installment,
                    'installment_duration' => $u->installment_duration ?? 6,
                    'installment_total_price' => $u->installment_total_price ?? 0,
                    'installment_deposit_percent' => $u->installment_deposit_percent ?? 30,
                    'installment_monthly_payment' => $u->installment_monthly_payment ?? 0,
        ...
        <div class="border-t border-slate-100 pt-8" x-data="{
            showLuxury: {{ old('has_luxury_layout', $property->has_luxury_layout) ? 'true' : 'false' }},
            units: {{ json_encode(old('units', $existingUnits->toArray())) }} || [],
            addUnit() {
                this.units.push({
                    id: null,
                    name: '',
                ...
                <div class="bg-slate-50 p-6 rounded-3xl border border-slate-100/70 text-left font-sans flex flex-col gap-4">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        <div class="flex flex-col gap-2">
                            ...
                            <input type="text" name="hero_video_url" value="{{ old('hero_video_url', $property->hero_video_url) }}" placeholder="e.g. /lumiere/lumiere-bg-video.mp4" class="w-full bg-white border border-slate-200 rounded-xl px-4 py-3 text-xs text-slate-700 focus:outline-none focus:ring-2 focus:ring-[#0d6e60] transition-all font-sans font-sans">
                        </div>
                        <div class="flex flex-col gap-2">
                            <label class="text-xs font-bold text-slate-400">Or Upload Video File</label>
                            <input type="file" name="hero_video_file" accept="video/*" class="w-full bg-white border border-slate-200 rounded-xl px-4 py-2.5 text-xs text-slate-700 focus:outline-none focus:ring-2 focus:ring-[#0d6e60] transition-all font-sans">
                            @if($property->hero_video_url && !str_contains($property->hero_video_url, 'lumiere'))
                                <div class="text-[10px] text-slate-400 font-medium mt-1">
                                    Current file: <span class="font-mono">{{ basename($property->hero_video_url) }}</span>
                                </div>
                            @endif
                        </div>
                    </div>
                            ...
                            <div class="bg-slate-50 border border-slate-100 p-6 rounded-3xl space-y-4 text-left">
                                ...
                                <div class="grid grid-cols-2 gap-4">
                                    <div class="flex flex-col gap-1.5">
                                        ...
                                        <input type="text" name="perspectives[{{ $key }}][title]" value="{{ old("perspectives.{$key}.title", $existingPersp ? $existingPersp->title : $defaultTitle) }}" class="w-full bg-white border border-slate-200 rounded-xl px-4 py-2.5 text-xs text-slate-700 focus:outline-none focus:ring-2 focus:ring-[#0d6e60]">
                                    </div>
                                    <div class="flex flex-col gap-1.5">
                                        <label class="text-[10px] font-bold text-slate-400 uppercase">Subtitle</label>
                                        <input type="text" name="perspectives[{{ $key }}][subtitle]" value="{{ old("perspectives.{$key}.subtitle", $existingPersp ? $existingPersp->subtitle : 'Perspective') }}" class="w-full bg-white border border-slate-200 rounded-xl px-4 py-2.5 text-xs text-slate-700 focus:outline-none focus:ring-2 focus:ring-[#0d6e60]">
                                    </div>
                                </div>

                                <div class="flex flex-col gap-1.5">
                                    <label class="text-[10px] font-bold text-slate-400 uppercase">Image URL or Path</label>
                                    <input type="text" name="perspectives[{{ $key }}][image_url]" value="{{ old("perspectives.{$key}.image_url", $existingPersp ? $existingPersp->image_path : "") }}" placeholder="/lumiere/{{$key}}-view-night.png" class="w-full bg-white border border-slate-200 rounded-xl px-4 py-2.5 text-xs text-slate-700 focus:outline-none focus:ring-2 focus:ring-[#0d6e60] font-sans">
                                </div>
                                ...
                                <div class="flex flex-col gap-1.5">
                                    ...
                                    @if($existingPersp && !str_contains($existingPersp->image_path, 'lumiere'))
                                        <div class="text-[10px] text-slate-400 font-medium mt-1">
                                            Current file: <span class="font-mono">{{ basename($existingPersp->image_path) }}</span>
                                        </div>
                                    @endif
                                </div>
                                ...
                                <div class="flex flex-col gap-1.5">
                                    ...
                                    <textarea name="perspectives[{{ $key }}][description]" rows="3" class="w-full bg-white border border-slate-200 rounded-xl px-4 py-2.5 text-xs text-slate-700 focus:outline-none focus:ring-2 focus:ring-[#0d6e60] font-sans">{{ old("perspectives.{$key}.description", $existingPersp ? $existingPersp->description : "") }}</textarea>
                                </div>
                            </div>
                        @endforeach
                    </div>
                ...
                <div class="space-y-6">
                    <div class="flex justify-between items-center text-left font-sans">
                        <div>
                            ...
                            <p class="text-[10px] text-slate-400 font-medium font-sans font-sans">Add layouts, outright purchase prices, financing terms, and custom hotspots.</p>
                        </div>
                        ...
                        <template x-for="(unit, index) in units" :key="index">
                            <div class="bg-slate-50 border border-slate-100 rounded-3xl p-6 sm:p-8 space-y-6 text-left relative font-sans shadow-sm">
                                ...
                                <div class="flex flex-col gap-1.5">
                                    ...
                                    <template x-if="unit.image_url && !unit.image_url.includes('lumiere')">
                                        <div class="text-[10px] text-slate-400 font-medium mt-1">
                                            Current image: <span class="font-mono" x-text="unit.image_url.split('/').pop()"></span>
                                        </div>
                                    </template>
                                </div>
                                ...
                                <div class="border-t border-slate-200/60 pt-6 mt-6">
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <template x-for="(spot, hIndex) in unit.hotspots" :key="spot.id">
                                            <div class="bg-white border border-slate-200 p-4 rounded-2xl relative space-y-3 font-sans shadow-sm font-sans">
                                                <button type="button" @click="unit.hotspots.splice(hIndex, 1)" class="absolute top-3 right-3 text-[10px] font-extrabold text-rose-500 hover:text-rose-600 uppercase focus:outline-none font-sans">
                                                    Delete
                                                </button>
                                                ...
                                                <div class="flex flex-col gap-1">
                                                    <label class="text-[9px] font-bold text-slate-400 uppercase font-sans">Hotspot Title</label>
                                                    <input type="text" :name="'units['+index+'][hotspots]['+hIndex+'][title]'" x-model="spot.title" placeholder="e.g. Integrated Compact Kitchenette" required class="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-1.5 text-xs text-slate-700 focus:outline-none font-sans">
                                                </div>
                                        ...
                                        <div x-show="!unit.hotspots || unit.hotspots.length === 0" class="border border-dashed border-slate-200 rounded-2xl py-6 text-center text-slate-400 text-[11px] font-semibold md:col-span-2 select-none bg-white/40 font-sans">
